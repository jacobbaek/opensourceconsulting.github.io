...
GitHub Page를 선택했습니다. 우리는 `글 쓰기에` 집중하면서 무료 테마나 오픈소스로 Github에서 서비스 하면 됩니다.  
우선 다른 블로그와 달리 `Git`을 조금 알아야 하구요.
`MarkDown`도 알아야 합니다. 어렵지 않습니다.  
이 글이 도움이 되길 바랍니다.  

## GitHub 회원가입
아직 가입 안하셨다면, 이번 기회에 가입해 보세요  
오픈소스컨설팅 기술 블로그로 감을 익히시고 `개인 GitHub 블로그` 운영도 해보세요 :)

## 환경 구성
Windows, Mac(Linux) 환경 구성 방법을 안내해 드리겠습니다.  
환경 구성 후 GitHub에서 블로그 소스코드를 내려 받고,  
글을 편집하고 GitHub에 올려 보겠습니다.
글 편집에는 Atom이라는 도구를 이용했지만 자유롭게 어떤 편집기든 가능합니다.(심지어 메모장 가능!)  
이 과정이 어려우시면 오픈소스컨설팅 직원은 `https://wiki.osci.kr/pages/viewpage.action?pageId=41553358`에 올려주세요.
  우리 엔지니어들이 도와 드릴겁니다. 글이 중요하지 뭐가 중요하겠어요. 
